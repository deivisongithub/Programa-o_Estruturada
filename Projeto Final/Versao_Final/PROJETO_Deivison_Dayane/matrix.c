 ...
 * ALUNA: DAYANE FREITAS FELIPE RAMOS             MATRICULA: 20190093910 */
...
void print_matrix(struct matrix a_matrix){
    ...
    int acumulador = 0;
    for(int k = 0;k < (a_matrix.n_rows * a_matrix.n_cols);k++){
        a_matrix.data[k] = a_matrix.data[k + a_matrix.offset];
    }
...
struct matrix slice(struct matrix a_matrix, int rs, int re, int cs, int ce){
    ...
    index_start = ((rs - 1)*a_matrix.stride_rows) + (cs - 1);
    index_end = ((re - 1)*a_matrix.stride_rows) + (ce - 1);
    

    a_matrix.offset = index_start;

    if(ce != cs){
        a_matrix.stride_cols = 1;
    }
    else{
        a_matrix.stride_cols = 0;
    }
...
float mean(struct matrix a_matrix){
    ...
    float soma_valores = sum(a_matrix);
